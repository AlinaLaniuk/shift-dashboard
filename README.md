...
Для работы с графиками выбрала библиотеку Apache ECharts. При выборе библиотеки исходила из следующих соображений:
- умеет ли библиотека в диаграмму Ганта.
...
- Recharts, Nivo - тяжелая реализация диаграммы Ганта, слабая кастомизация.
- Highcharts - все, что нужно, поддерживает из коробки, но требует соблюдения проприетарной лицензии. Возможно, это не было бы проблемой для реализации, но не хотелось сюрпризов посреди разработки с поддержкой встраиваемых плагинов и т.д.
- VisX - мощная и гибкая библиотека, но для требований задания избыточна - требует написания избыточной кодовой инфраструктуры, фактически мини-engine для создания графиков.

ECharts по совокупности характеристик оказался наиболее подходящим вариантом, легко встраивается в проект на React, имеет подробную документацию и много примеров.
...
- mocks - здесь лежат непосредственно моки, а также настройки msw. 'базой данных' служит localStorage.
- types - типы для апи и структур данных для графиков.
- utils - хелперы для работы с временем.
Redux не использовала. Для поставленных задач достаточно стейта.
...
- Дашборд поддерживает зум. Зум сделан двух видов: по колесику (scroll event) и в виде горизонтального скроллбара. Дашборд корректно отображается на разных расширениях и графики читаемы благодаря зуму. На совсем маленьких экранах возможны проблемы с визуализацией, но вряд ли здесь нужна поддержка на мобилках.
